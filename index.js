...
let allowedOrigins = [
  'http://localhost:8080',
  'http://testsite.com',
  'https://afternoon-badlands-59179.herokuapp.com//',
  'http://localhost:1234',
];
app.use(
  cors({
...
//Return data JSON about a movie by movie name
app.get(
  '/movies/:movieTitle',
  passport.authenticate('jwt', { session: false }),
  (req, res) => {
...
app.get(
  '/movies/genres/:movieGenre',
  passport.authenticate('jwt', { session: false }),
  (req, res) => {
    Movies.findOne({ 'Genre.Name': req.params.movieGenre })
      .then((movie) => {
...
app.put(
  ...
  (req, res) => {
    ...
    Users.findOneAndUpdate(
      ...
      { new: true }, //making sure the updated doc is returned
      (err, updatedUser) => {
        if (err) {
          console.error(err);
          res.status(500).send('Error: ' + err);
...
//Allow users to add to their favorite movies list
app.post(
